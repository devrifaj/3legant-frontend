...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    updateQuantity(state, action: PayloadAction<{ id: string; quantity: number }>) {
      ...
      if (item) {
        item.quantity = Math.min(MAX_QUANTITY, Math.max(MIN_QUANTITY, action.payload.quantity));
      }
    },
...
export const selectShippingCost = (state: { cart: CartState }) => {
  ...
  const option = state.cart.shippingOptions.find((opt) => opt.id === state.cart.selectedShipping);
